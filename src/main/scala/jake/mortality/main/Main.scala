...
*
    ...
    val notes = sqlContext.load("jdbc", Map("driver" -> "org.postgresql.Driver", "url" -> "jdbc:postgresql://mimic3-1.coh8b3jmul4y.us-west-2.rds.amazonaws.com:5432/MIMIC?user=jfund&password=G0tbeared", "dbtable" -> "MIMICIII.NOTEEVENTS")).limit(3000)
    ...
    val filteredNotes = noteJoin.filter((noteJoin("intime") + expr("INTERVAL 6 HOUR") >
      noteJoin("charttime")) && (noteJoin("intime") + expr("INTERVAL 6 HOUR") < noteJoin("outtime")))
    val features = Topics.run(sqlContext, filteredNotes, 20, 500)

    val adm = sqlContext.load("jdbc", Map("url" -> "jdbc:postgresql://mimic3-1.coh8b3jmul4y.us-west-2.rds.amazonaws.com:5432/MIMIC?user=jfund&password=G0tbeared",
    ...
    val vectored = joinedAdmFeatures.select("deathtime","features").map(row => LabeledPoint(row.getInt(0), Vectors.dense(row.getSeq(1).toArray[Double])))
